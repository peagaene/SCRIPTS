...
def _place_text_center(ms, content: str, x: float, y: float, height: float, rotation_deg: float, layer: str, style: str):
...
def _add_dim_aligned(ms, p1, p2, offset_sign, offset_mag, layer, text_height, arrow_size, dim_decimals_or_flag, style_texto):
    if p1 is None or p2 is None:
        return None
...
def _dedup_merge_lines(eixos: List[LineString]) -> List[LineString]:
    ...
    try:
        ...
        out, seen = [], set()
        for ls in eixos:
            sig = (round(ls.bounds[0], 3), round(ls.bounds[1], 3), round(ls.bounds[2], 3), round(ls.bounds[3], 3), round(ls.length, 3))
            if sig not in seen:
                seen.add(sig)
                out.append(ls)
        return out
...
def _closest_hits(center: Point, n_unit: Tuple[float, float], span: float, geom) -> Tuple[Tuple[float, float], Tuple[float, float]] | None:
    ...
    if gt == "Point":
        ...
        try:
            ...
            logger.debug(f"Falha ao extrair pontos da intersecao: {e}")
    if not pts:
        return None
...
class RoadMeasurementProcessor:
    """Mede e rotula vias."""
    ...
    def __init__(
        ...
        cross_span_m: float,
        amostras_fracs: tuple,
        nome_offset_m: float,
        nome_offset_side: str,
        nome_sufixo: str,
        style_texto: str,
        dim_text_style: str | None,
        layer_via_medida: str,
        layer_via_nome: str,
        sample_mode: str,
        offset_lote_lote_extra_m: float,
        dim_gap_m: float,
        dim_min_len_m: float,
        dim_min_spacing_m: float,
        dim_max_por_trecho: int,
        dim_max_dist_m: float,
        dim_min_sep_area_m: float,
        dim_equal_tol_m: float,
        nome_offset_add_dim_m: float | None,
        nome_side_mode: str,
        nome_shift_along_m: float,
        nome_case: str,
    ):
        self.texto_altura = texto_altura
        self.offset_texto_m = offset_texto_m
        self.cross_span_m = cross_span_m
        self.amostras_fracs = amostras_fracs
        self.nome_offset_m = nome_offset_m
        self.nome_offset_side = nome_offset_side
        self.nome_sufixo = nome_sufixo
        self.style_texto = style_texto
        self.dim_text_style = dim_text_style or style_texto
        self.layer_via_medida = layer_via_medida
        self.layer_via_nome = layer_via_nome
        self.sample_mode = sample_mode
        self.offset_lote_lote_extra_m = offset_lote_lote_extra_m
        self.dim_gap_m = dim_gap_m
        self.dim_min_len_m = dim_min_len_m
        self.dim_min_spacing_m = dim_min_spacing_m
        self.dim_max_por_trecho = dim_max_por_trecho
        self.dim_max_dist_m = dim_max_dist_m
        self.dim_min_sep_area_m = dim_min_sep_area_m
        self.dim_equal_tol_m = dim_equal_tol_m
        self.nome_offset_add_dim_m = nome_offset_add_dim_m
        self.nome_side_mode = nome_side_mode
        self.nome_shift_along_m = nome_shift_along_m
        self.nome_case = nome_case

    def process(self, ms, eixos: List[LineString], bordas_guia: List[LineString], lotes_polygons: List[Polygon], name_provider: Optional[Callable[[float, float], str]] = None):
        for lyr in [self.layer_via_medida, self.layer_via_nome]:
        ...
        arrow_size_factor = float(getattr(TOLERANCES, "DIM_ARROW_SIZE_FACTOR", 0.5))
        span = max(float(self.cross_span_m), 1.0)
        ...
        for idx_full, eixo_full in enumerate(eixos_proc, start=1):
            ...
            if eixo_full.length >= 5.0:
                try:
                    ...
                    logger.debug(f"Falha ao calcular ponto medio do eixo: {e}")
                    mid_all = eixo_full.interpolate(0.5, normalized=True)
                nome = None
                if name_provider:
                    try:
                        ...
                        logger.debug(f"Falha ao obter nome da via: {e}")
                        nome = None
                if not nome:
                    nome = f"VIA-{idx_full:03d}"
                if self.nome_sufixo:
                    nome = f"{nome}{self.nome_sufixo}"
                if str(self.nome_case).lower() == "upper":
                    nome = nome.upper()
                ...
                if self.nome_side_mode == "oposto_dim":
                    side = +1.0
                else:
                    side = +1.0
                    if isinstance(self.nome_offset_side, (int, float)):
                        side = float(self.nome_offset_side)
                    elif isinstance(self.nome_offset_side, str) and self.nome_offset_side.strip() != "auto":
                        side = +1.0 if self.nome_offset_side.strip() == "+1" else -1.0
                    else:
                        if lotes_union is not None:
                            test_pt = Point(mid_all.x + nx_nom * self.nome_offset_m, mid_all.y + ny_nom * self.nome_offset_m)
                            try:
                                if lotes_union.buffer(0).contains(test_pt):
                                    side = -1.0
                            except Exception as e:
                                logger.debug(f"Falha ao checar lado do nome da via: {e}")
                                side = +1.0

                nome_extra = self.offset_texto_m if self.nome_offset_add_dim_m is None else float(self.nome_offset_add_dim_m)
                nome_offset_total = float(self.nome_offset_m) + float(nome_extra)

                base_x = float(mid_all.x + tx * float(self.nome_shift_along_m))
                base_y = float(mid_all.y + ty * float(self.nome_shift_along_m))
                ...
                try:
                    _place_text_center(
                        ...
                        float(self.texto_altura),
                        rotation_deg=ang_nome,
                        layer=self.layer_via_nome,
                        style=self.style_texto,
                    )
                except Exception as e:
                    logger.debug(f"Falha ao inserir nome da via: {e}")

            if self.sample_mode == "por_trecho":
                coords = list(eixo_full.coords)
                trechos = []
                for i in range(len(coords) - 1):
            ...
            min_sep2 = float(self.dim_min_sep_area_m) ** 2 if self.dim_min_sep_area_m else 0.0
            ...
            for eixo in trechos:
                try:
                    ...
                    eixo_len = 0.0
                if eixo_len < float(self.dim_min_len_m):
                    continue

                if self.dim_min_spacing_m and float(self.dim_min_spacing_m) > 0:
                    max_by_len = max(1, int(eixo_len / float(self.dim_min_spacing_m)))
                    n = max(1, min(int(self.dim_max_por_trecho), max_by_len))
                    fracs_eff = np.linspace(0.20, 0.80, n).tolist()
                else:
                    fracs_eff = list(self.amostras_fracs)
                ...
                for frac in fracs_eff:
                    ...
                    gap = max(0.1, float(self.dim_gap_m))
                    off_gg = float(self.offset_texto_m)
                    off_gl = float(self.offset_texto_m) + gap
                    off_ll = float(self.offset_texto_m) + 2.0 * gap + float(self.offset_lote_lote_extra_m)

                    max_dist = float(self.dim_max_dist_m)
                    dist_gg = float(Point(g_neg).distance(Point(g_pos)))
                    eq_tol = float(self.dim_equal_tol_m)
                    if dist_gg > max_dist:
                        continue
                    ...
                    _add_dim_aligned(
                        ...
                        -1.0,
                        off_gg,
                        self.layer_via_medida,
                        float(self.texto_altura),
                        arrow_size_factor * float(self.texto_altura),
                        2,
                        self.dim_text_style,
                    )
...
def medir_e_rotular_vias(
    ...
    """
    Mede e rotula vias com dimensoes guia-guia, guia-lote e lote-lote.

    Mantem assinatura do fluxo legado.
    """
    processor = RoadMeasurementProcessor(
        texto_altura=texto_altura,
        offset_texto_m=offset_texto_m,
        cross_span_m=cross_span_m,
        amostras_fracs=amostras_fracs,
        nome_offset_m=nome_offset_m,
        nome_offset_side=nome_offset_side,
        nome_sufixo=nome_sufixo,
        style_texto=style_texto,
        dim_text_style=dim_text_style,
        layer_via_medida=layer_via_medida,
        layer_via_nome=layer_via_nome,
        sample_mode=sample_mode,
        offset_lote_lote_extra_m=offset_lote_lote_extra_m,
        dim_gap_m=dim_gap_m,
        dim_min_len_m=dim_min_len_m,
        dim_min_spacing_m=dim_min_spacing_m,
        dim_max_por_trecho=dim_max_por_trecho,
        dim_max_dist_m=dim_max_dist_m,
        dim_min_sep_area_m=dim_min_sep_area_m,
        dim_equal_tol_m=dim_equal_tol_m,
        nome_offset_add_dim_m=nome_offset_add_dim_m,
        nome_side_mode=nome_side_mode,
        nome_shift_along_m=nome_shift_along_m,
        nome_case=nome_case,
    )
    processor.process(ms, eixos, bordas_guia, lotes_polygons, name_provider=name_provider)


__all__ = ["RoadMeasurementProcessor", "medir_e_rotular_vias"]
