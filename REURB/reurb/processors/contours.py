...
def _ensure(doc, layer: str):
    try:
        ...
        logger.debug(f"Falha ao garantir layer {layer}: {e}")
...
def _ensure_text_style(doc: ezdxf.EzDxf, style_name: str):
    try:
        ...
        logger.debug(f"Falha ao garantir estilo de texto {style_name}: {e}")
...
def _as_gdal_dataset(mdt_src):
    ...
    try:
        ...
        logger.debug(f"Falha ao abrir dataset GDAL: {e}")
        return mdt_src
...
def _tangent_angle_deg(ls: LineString, dist: float) -> float:
    ...
    a = max(0.0, dist - TANGENT_DELTA)
    b = min(ls.length, dist + TANGENT_DELTA)
    pa = ls.interpolate(a)
    pb = ls.interpolate(b)
    dx, dy = (pb.x - pa.x), (pb.y - pa.y)
    if abs(dx) < TANGENT_EPS and abs(dy) < TANGENT_EPS:
        return 0.0
...
def gerar_curvas_nivel(ms, mdt_src, params, clip_poly: Optional[Polygon] = None):
    """Gera curvas de nivel (intermediarias/mestras) e rotulos no DXF."""
    ...
    def _draw_polyline(coords, layer, elevation: float | None = None):
        ...
        try:
            ...
            logger.debug(f"Falha ao definir elevacao na curva: {e}")
        return pl
    ...
    def _add_text(content: str, x: float, y: float, angle_deg: float):
        try:
            ...
            logger.debug(f"Falha ao inserir texto de curva: {e}")
        ...
        for ls in lines:
            ...
            for s in label_positions:
                ...
                if ends_only and ls.length > 0.0:
                    shift_ref = max(SHIFT_REF_MIN, min(1.0, ls.length * SHIFT_REF_FACTOR))
                    if s_original <= 1e-6:
    ...
    try:
        ...
        logger.debug(f"Falha ao liberar dataset temporario de curvas: {e}")
