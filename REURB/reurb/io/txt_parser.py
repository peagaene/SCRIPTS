...
def _detect_sep(txt: str) -> str:
    ...
    try:
        ...
        logger.debug(f"Falha ao detectar separador via Sniffer: {e}")
...
def ler_bruto(path_txt: str, sep: Optional[str] = None) -> pd.DataFrame:
    ...
    if "idx" in df.columns:
        try:
            ...
            logger.debug(f"Falha ao avaliar coluna idx: {e}")
    ...
    try:
        ...
        logger.debug(f"Falha ao aplicar heuristica E/N por milhar: {e}")
