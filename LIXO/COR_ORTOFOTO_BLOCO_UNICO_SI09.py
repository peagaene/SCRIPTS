...
formato = '.tif'
dir_fotos = r'\\192.168.2.29\d\SI_04\SI_09\SI04_IR\Nova pasta'
dir_output = r'\\192.168.2.29\d\SI_04\SI_09\SI04_IR\Nova pasta\Nova pasta'
...
for i in tqdm(range(len(lista_dir)), desc="Processando imagens", unit="imagem"):
    ...
    process_image(
        ...
        gamma=1,               # Ajuste de gama
        brightness_adjustment=2,  # Ajuste de brilho
        contrast=1.0,            # Ajuste de contraste
        intensity_factor=1,    # Ajuste de intensidade
        red=-2, green=-2, blue=-2, # Ajuste individual dos canais RGB
        saturation_factor=1,   # Aumentar saturação
