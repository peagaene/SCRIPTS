...
def build_pdal_crop_pipeline(input_files: List[str], crop_polygon: Polygon, out_file: str, change_to_class: Optional[int] = None) -> str:
    ...
    # Optimized writer settings for speed (LAS format)
    writer = {
        "type": "writers.las", 
        "filename": out_file,
...
def process_one_feature(feat: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Create both output files (LAS format)
    ...
    out_path_default = os.path.join(block_config.output_folder_default, f"{out_name_default}.las")
    out_path_classified = os.path.join(block_config.output_folder_classified, f"{out_name_classified}.las")
    ...
    try:
        ...
        # Export default points (all points changed to class 1)
        ...
        # Export all classified points (keep original classifications)
        ...
        # Convert LAS to LAZ if requested
        if params.convert_to_laz:
            print(f"🔄 Convertendo LAS para LAZ: {os.path.basename(out_path_default)}")
            convert_las_to_laz(out_path_default)
            convert_las_to_laz(out_path_classified)
            
            # Update output filenames to reflect LAZ files
            laz_default = out_path_default.replace('.las', '.laz')
            laz_classified = out_path_classified.replace('.las', '.laz')
            result["saida_filename"] = f"{laz_default}; {laz_classified}"
...
def build_gui_and_run():
    if _GUI_LIB == "psg":
        ...
        # Simplified GUI for now - single block mode
        layout = [
            ...
            [sg.Text("Pasta saída - pontos default (classe 1) - arquivos .las"), sg.Input(key="out_folder_default"), sg.FolderBrowse("Browse")],
            [sg.Text("Pasta saída - pontos classificados - arquivos .las"), sg.Input(key="out_folder_classified"), sg.FolderBrowse("Browse")],
            [sg.HSeparator()],
            ...
            [sg.Checkbox("Converter LAS para LAZ após processamento", key="convert_to_laz", default=False)],
        ...
        win = sg.Window("Corte LAZ → LAS por articulação (PDAL)", layout, finalize=True, resizable=True)
            ...
            if ev in (sg.WINDOW_CLOSED, "Fechar"):
                ...
                try:
                    ...
                    params = AppParams(
                        ...
                        convert_to_laz=bool(values.get("convert_to_laz", False)),
                    )
        ...
        out_folder_default = filedialog.askdirectory(title="Pasta saída - pontos default (classe 1)")
        out_folder_classified = filedialog.askdirectory(title="Pasta saída - pontos classificados")
        lote = ""; bloco = ""
        try:
            lote = input("Lote (XX): ")
            bloco = input("Bloco (Y): ")
