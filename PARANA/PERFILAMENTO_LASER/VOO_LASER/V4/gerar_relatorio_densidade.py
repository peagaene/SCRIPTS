...
def calcular_densidade(las_path, tamanho_m=50, modo="todos", fator=1):
    ...
    for _, cx, cy in regioes:
        dentro = (x >= cx - metade_x) & (x <= cx + metade_x) & (y >= cy - metade_y) & (y <= cy + metade_y)
...
def gerar_relatorio_densidade(dir_execucao_voo, blocos_info, datas_filtro, log_func=None):
    for info in blocos_info:
        ...
        dir_saida_lote = os.path.join(dir_lote, "DENSIDADE_LAS")
        os.makedirs(dir_saida_lote, exist_ok=True)
        ...
        for pasta in os.listdir(dir_bloco):
            ...
            for subpasta in os.listdir(caminho_voo):
                ...
                for arquivo in os.listdir(caminho_rt):
                    ...
                    if log_func:
                        log_func(f"[INFO] Processando {bloco_nome} / {pasta} / {arquivo}\n")
                    try:
        ...
        if densidades_voo:
            df_final = (
                pd.DataFrame.from_dict(
                    densidades_voo,
                    orient="index",
                    columns=["Densidade Q1 (ppm2)", "Q2", "Q3", "Q4"],
                )
                .reset_index()
                .rename(columns={"index": "Data_Voo"})
            )

            caminho_saida = os.path.join(dir_saida_lote, f"densidade_{bloco_nome}.xlsx")
            df_final.to_excel(caminho_saida, index=False)
            if log_func:
                log_func(f"[OK] Densidade salva: {caminho_saida}\n")
        else:
            if log_func:
                log_func(f"[AVISO] Nenhum dado para o bloco {bloco_nome}\n")
