# === PROCESSAMENTO — LÓGICA ORIGINAL + PATCHES DE PERFORMANCE ===
# ➤ TIF primeiro (reprojeta se preciso, recorta, remove alpha/copia)
# ➤ JP2 depois (fora de IMAGENS_TRANSFORMADAS), com GMLJP2/GeoJP2 só se tiver georef
# ➤ Only-JP2: ignora processamento e gera JP2 direto do original
# ➤ UI: pares com fila dinâmica, “Enfileirar este/Todos”, status; menos spam de logs
# ➤ Patches: fecha datasets sempre, sindex + unary_union local, GDAL cache↑, OPJ threads↓, logs em lote, gc periódico
...
def gdal_translate_drop_alpha_or_copy(src_path, dst_path, force_event, window, get_total, processados):
    """Se >=4 bandas, pega 1-2-3; se 1-3 bandas, só copia (GTiff tiled, compressão None)."""
    ...
    # Compressão do TIF = None (como solicitado)
...
def compute_output_dirs(img_path, in_root, out_root):
    """base_out = .../IMAGENS_TRANSFORMADAS ; jp2_dir = pasta-irmã .../jp2"""
    if out_root:
    ...
    jp2_dir  = os.path.join(parent, JP2_SUBFOLDER_NAME)  # fora de IMAGENS_TRANSFORMADAS
    return base_out, jp2_dir

# === WORKER (consome a fila de pares) ===
def worker_process_queue(pairs_q: Queue, shapefile, only_jp2, gerar_jp2_flag, substituir_original,
    ...
    try:
        ...
        if not only_jp2:
            ...
            try:
                sindex = gdf.sindex  # pode ser None se não houver backend
            except Exception:
                sindex = None
        else:
        ...
        while not stop_event.is_set():
            ...
            for dp, _, fs in os.walk(in_root):
                for f in fs:
                    ...
                    base_out, jp2_dir = compute_output_dirs(img, in_root, out_root)
                    ...
                    saida_jp2   = os.path.join(jp2_dir, f"{base_nome}.jp2")

                    # === ONLY-JP2 ===
                    if only_jp2:
                        try:
                            os.makedirs(jp2_dir, exist_ok=True)
                            if os.path.exists(saida_jp2):
                    ...
                    # 2) Recorte (sindex + unary_union local)
                    try:
                        with rasterio.open(src_for_clip) as src:
                            ...
                            if not geom_raster.intersects(geom_unificada):
                                ...
                                # limpeza
                                pass  # segue adiante para contabilizar
                                ...
                                if sindex is not None:
                                    ...
                                    if cand_idx:
                                        ...
                                        intersecao = sub[sub.geometry.intersects(geom_raster)]
                                    else:
                                        intersecao = gdf.iloc[[]]
                                else:
                                    # fallback sem sindex
        ...
        # (usa write_event_value para não travar thread)
        # se sobrou algo no buffer:
        # (protege contra corrida: se já flushou, len==0)
        try:
            if log_buffer:
                window.write_event_value(EVT_LOG, "\n".join(log_buffer))
                log_buffer.clear()
...
layout = [
    [sg.Frame("Pares (Entrada → Saída base)", [
        [pairs_col],
    ...
    [sg.Checkbox("Gerar JP2 apenas (≈1:10) em pasta 'jp2'", default=True, key="-JP2-")],
...
window = sg.Window("Processamento de Imagens — GDAL (TIF antes do JP2, otimizado)", layout,
...
def start_worker(only_jp2, gerar_jp2_flag, substituir_original, shp):
    global worker_th, start_time
    start_time = time.time()
    worker_th = threading.Thread(
        target=worker_process_queue,
        args=(pairs_q, shp, only_jp2, gerar_jp2_flag, substituir_original,
...
while True:
    ...
    if event == "-START-":
        ...
        only_jp2 = (gerar_jp2_flag and not substituir_original)
        ...
        start_worker(only_jp2, gerar_jp2_flag, substituir_original, shp)
